...
export async function exportAsZIP(
  ...
  const thoughts: Thought[] = await thoughtRepository.listNewestFirst(9999);
  const zip = new JSZip();

  // thoughts.json
  zip.file(
    'thoughts.json',
    JSON.stringify({ exportedAt: new Date().toISOString(), version: 1, thoughts }, null, 2),
  );
